...
async def test_list_members_includes_owner(
    client: AsyncClient, auth_cookies, test_user, project
):
...
async def test_list_members_total_reflects_all_members(
    ...
    await _add_member(client, auth_cookies, project.project_key, user_a.email)
...
async def test_add_member(
    ...
    response = await _add_member(
        client, auth_cookies, project.project_key, new_user.email, role="viewer"
    )
...
async def test_add_member_with_member_role(
    ...
    response = await _add_member(
        client, auth_cookies, project.project_key, new_user.email, role="member"
    )
    assert response.status_code == 201
...
async def test_add_owner_as_member_fails(
    ...
    response = await _add_member(
        client, auth_cookies, project.project_key, test_user.email
    )
...
async def test_add_duplicate_member_fails(
    ...
    await _add_member(client, auth_cookies, project.project_key, new_user.email)

    response = await _add_member(
        client, auth_cookies, project.project_key, new_user.email
    )
    assert response.status_code == 409
...
async def test_add_member_with_owner_role_fails(
    ...
    response = await _add_member(
        client, auth_cookies, project.project_key, new_user.email, role="owner"
    )
...
async def test_add_member_nonexistent_user_fails(
    ...
    response = await _add_member(
        client, auth_cookies, project.project_key, "nonexistent@example.com"
    )
    assert response.status_code == 404
...
async def test_remove_member(client: AsyncClient, auth_cookies, project, db_session):
...
async def test_member_can_read_project(
    ...
    await _add_member(
        client, auth_cookies, project.project_key, member_user.email, role="viewer"
    )
...
async def _make_member_cookies(
    client: AsyncClient,
    auth_cookies: dict[str, str],
    project,
    db_session,
    ...
    """Create a user with member role and return (user, auth_cookies)."""
...
async def test_member_role_user_cannot_add_member(
    ...
    _member_user, member_cookies = await _make_member_cookies(
        client, auth_cookies, project, db_session, "member-add@example.com"
    )
    another = await create_user(db_session, email="another-add@example.com")
    response = await _add_member(
        client, member_cookies, project.project_key, another.email
    )
    assert response.status_code == 403
...
async def test_member_role_user_cannot_remove_member(
    ...
    _member_user, member_cookies = await _make_member_cookies(
        client, auth_cookies, project, db_session, "member-rem@example.com"
    )
    another = await create_user(db_session, email="another-rem@example.com")
    await _add_member(client, auth_cookies, project.project_key, another.email)
    response = await client.delete(
...
async def test_member_role_user_cannot_update_role(
    ...
    _member_user, member_cookies = await _make_member_cookies(
        client, auth_cookies, project, db_session, "member-upd@example.com"
    )
