...
async def test_get_metrics_summary(
    client: AsyncClient, auth_cookies, project_with_data
):
...
async def test_get_metrics_time_series(
    client: AsyncClient, auth_cookies, project_with_data, granularity
):
    response = await client.get(
...
async def test_metrics_pagination(client: AsyncClient, auth_cookies, project_with_data):
...
async def test_metrics_range_too_long(client: AsyncClient, auth_cookies, project):
...
async def test_metrics_range_too_short(client: AsyncClient, auth_cookies, project):
    start = datetime.now(UTC)
...
async def test_self_metrics_are_recorded_when_enabled(
    client: AsyncClient, auth_cookies, session_factory, monkeypatch
):
    from app import models
    ...
    response = await client.get(request_path, cookies=auth_cookies)
    assert response.status_code == 200
